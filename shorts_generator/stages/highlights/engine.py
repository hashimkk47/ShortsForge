"""Rank the most viral-worthy highlights in a transcript.

The engine is LLM-agnostic: the model call is injected via ``llm_fn`` (any
``Callable[[str], str]``), so the exact same prompts and post-processing drive
either the hosted MuAPI model (API mode) or a local OpenAI / Gemini / Ollama
model (local mode).

Pipeline:
  1. classify content type + density (tunes the prompt),
  2. chunk long videos with overlap so cross-boundary clips aren't missed,
  3. rank candidates against an explicit virality framework,
  4. dedupe overlapping candidates, keeping the higher score.
"""
...
from ...types import Highlight, Transcript
...
CHUNK_OVERLAP_SECONDS = 60      # overlap so cross-boundary clips aren't missed
MAX_HIGHLIGHT_API_ATTEMPTS = 3  # retries on invalid model JSON
...
def _sanitize_highlights(raw_highlights: object, duration: float) -> List[Highlight]:
    """Normalise model output into the expected shape; skip invalid entries."""
...
# ---------------------------------------------------------------------------
# Stages of the highlight engine
# ---------------------------------------------------------------------------
def detect_content_type(transcript: Transcript, llm_fn: LLMFn) -> Dict[str, str]:
    """Classify content type + density from a transcript sample."""
...
def build_transcript_text(transcript: Transcript) -> str:
    """Flatten a transcript into ``[t.ts] text`` lines for the ranking prompt."""
    segments = transcript.get("segments", [])
    return "\n".join(f"[{s['start']:.1f}s] {s['text'].strip()}" for s in segments)


def chunk_transcript(transcript: Transcript) -> List[Transcript]:
    """Split a long transcript into overlapping windows, each tagged with an offset."""
    segments = transcript.get("segments", [])
    ...
    while start < duration:
        ...
        if chunk_segs:
            ...
            chunk["_offset"] = start  # type: ignore[typeddict-unknown-key]
...
def rank_chunk(
    ...
    """Ask the LLM for ranked highlights in one transcript window."""
    # Request ~2x the target so dedupe has headroom, but cap the count so the
    # model doesn't have to emit a huge JSON payload (which can time out).
    ...
    for attempt in range(1, MAX_HIGHLIGHT_API_ATTEMPTS + 1):
        ...
        if attempt < MAX_HIGHLIGHT_API_ATTEMPTS:
            print(
                f"[highlights] invalid model output on attempt "
                f"{attempt}/{MAX_HIGHLIGHT_API_ATTEMPTS}; retrying",
                flush=True,
            )
            prompt = (
                base_prompt
    ...
    raise RuntimeError(
        f"Highlight generator produced invalid output after "
...
def get_highlights(transcript: Transcript, num_clips: int, llm_fn: LLMFn) -> List[Highlight]:
    """Rank highlights across a transcript and return them deduped, best first."""
    ...
    if duration >= LONG_VIDEO_THRESHOLD:
        ...
        for i, chunk in enumerate(chunks):
            offset = chunk.get("_offset", 0)  # type: ignore[typeddict-item]
        ...
        return dedupe_highlights(all_highlights)

    text = build_transcript_text(transcript)
    highlights = rank_chunk(text, content_info, duration, num_clips=num_clips, llm_fn=llm_fn)
    return dedupe_highlights(highlights)
