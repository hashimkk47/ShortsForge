"""Local-mode downloader — yt-dlp for remote URLs, pass-through for local files.

Always returns a local filesystem path so the rest of the local pipeline can
read the media straight off disk. Remote downloads are cached in the output
directory as ``source_<youtube_id>.mp4`` and reused on subsequent runs.
"""
...
_CACHE_EXTENSIONS = (".mp4", ".mkv", ".webm")
...
def download_youtube_local(source: str, fmt: str = "720", out_dir: Optional[str] = None) -> str:
    """Download a remote URL with yt-dlp, or return a local file path unchanged."""
    ...
    print(f"[download/local] {source} @ {fmt}p -> {out_dir}/", flush=True)
    ydl_opts = {
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        # merge_output_format may rename the extension after merging streams.
