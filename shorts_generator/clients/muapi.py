...
class MuAPIError(RuntimeError):
    """Raised for any non-transient MuAPI failure (bad status, timeout, etc.)."""
...
def submit(endpoint: str, payload: Dict[str, Any], retries: int = 3) -> str:
    """POST to ``/api/v1/{endpoint}`` and return the request id.

    Transient network errors are retried; HTTP 4xx/5xx responses fail fast.
    """
...
def fetch_result(request_id: str, retries: int = 3) -> Dict[str, Any]:
    """GET the latest result for a request id; retry on transient timeouts."""
...
def run(
    ...
    """Submit a job then poll to completion. Returns the final result payload."""
